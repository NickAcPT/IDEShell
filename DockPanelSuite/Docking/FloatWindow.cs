...
{
    public class FloatWindow : BaseThemedForm, INestedPanesContainer, IDockDragSource
    {
        protected override void OnMouseDown(MouseEventArgs e)
        {
            if (!TextBarRectangle.Contains(e.Location)) 
                goto mouseDown;
            ...
            switch (e.Button)
            {
                case MouseButtons.Left:
                    if (DockPanel.AllowEndUserDocking && this.AllowEndUserDocking)
                    {
                        Activate();
        ...
        public FloatWindow(DockPanel dockPanel, DockPane pane) : this()
        {
            InternalConstruct(dockPanel, pane, false, Rectangle.Empty);
        }

        public FloatWindow(DockPanel dockPanel, DockPane pane, Rectangle bounds) : this()
        {
            InternalConstruct(dockPanel, pane, true, bounds);
        }

        public void InternalConstruct(DockPanel dockPanel, DockPane pane, bool boundsSpecified, Rectangle bounds)
        {
            IsMdiContainer = true;
            DoubleBuffered = true;
            if (dockPanel == null)
        ...
        protected override void OnLayout(LayoutEventArgs levent)
        {
            ...
            RefreshChanges();
            var targetVisibility = (VisibleNestedPanes?.Count > 0);
            if (Visible != targetVisibility)
        ...
        {
            if (VisibleNestedPanes == null) return;
